...
def parse_args(args=None):
    ...
    parser.add_argument('--model', default='ModE', type=str)

    parser.add_argument('-n', '--negative_sample_size', default=128, type=int)
    parser.add_argument('-d', '--hidden_dim', default=500, type=int)
    parser.add_argument('-g', '--gamma', default=18, type=float)
    parser.add_argument('-a', '--adversarial_temperature', default=0.2, type=float)
    ...
    parser.add_argument('-save', '--save_path', default="save_model", type=str)
    ...
    parser.add_argument('-vct', type=str, default='TransE _dim500.json')
...
def save_model(model, optimizer, save_variable_list, args):
    ...
    entity_embedding = model.entity_embedding_random.detach().cpu().numpy()
    np.save(
        os.path.join(args.save_path, 'entity_embedding'),
        entity_embedding
    )

    relation_embedding = model.relation_embedding_random.detach().cpu().numpy()
    np.save(
...
def main(args):
    ...
    img_emb = torch.load('./visual/FB15K.pth')
    pretrained_file = './' + args.vct

    if args.model == 'ModE':
        kge_model = ModE(num_entity, num_relation, args.hidden_dim, args.gamma, args.img_dim, img_emb, pretrained_file=pretrained_file)
